...
module.exports = function (source, sourcemap) {
  ...
  if (config.extractStyle) {
    var extractStyleUrls;

    newSource = newSource.replace(stylesRegex, function (match, urls) {
      extractStyleUrls = replaceStringsWithRequires(urls).replace(/\[|\]/g, '').replace(/,/g, ';') + ';';

      // remove: stylesUrl: ['./foo.css', "./baz.css", "./index.component.css"]
      // add: require('./foo.css'); require("./baz.css"); require("./index.component.css");
      // to top of file
      return '//';
    });

    newSource = extractStyleUrls + "\n" + newSource;
